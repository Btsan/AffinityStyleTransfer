...
def test(encoder, decoder, name="test.mp4", imsize=256, davis=True, video_path='motorbike', style_path="styles/vangogh_starry_night.jpg", alpha=1.0):
	print("{} {} {} {} {}".format(name, davis, video_path, style_path, alpha))
	resize = transforms.Resize((imsize, imsize))
	if davis:
		dataset = ImageFolderDataset("datasets/DAVIS/JPEGImages/480p/" + video_path)
	...
	with torch.no_grad():
		...
		if alpha < 1:
			style_im = transforms.functional.resize(style_im, dataset[0].shape[-2:])
		style_ft = encoder(style_im)
		del style_im
		for idx, im in enumerate(dataloader):
			if im.dim() < 4:
				im = im.unsqueeze(0) 
			im = im.to(device) / 255
			content_ft = encoder(im)
			p = decoder(content_ft, style_ft, alpha=alpha)
			pred.append(p.cpu())
...
if __name__ == '__main__':
	...
	parser.add_argument('--davis', type=int, metavar='<int>', default=1, help='use davis dataset or not (default=1')
	parser.add_argument('--style', type=str, metavar='<.jpg>', default="styles/10313.jpg", help='style image to use (default="styles/10313.jpg"')
	...
	parser.add_argument('--alpha', type=float, metavar='<float>', default=1.0, help='stylization strength between 0-1 (default=1.0')
	...
	test(encoder, decoder, name=args.save, davis=bool(args.davis),video_path=args.video, style_path=args.style, alpha=args.alpha)